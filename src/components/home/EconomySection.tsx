  ...
  if (valid.length === 0) {
    return (
      <p className="text-sm text-neutral-400 py-6">
        {isRTL ? 'لا توجد أخبار حالياً' : 'Aucune actualité pour le moment'}
      </p>
    );
  }
    ...
    <div className="grid md:grid-cols-3 gap-5">
      {valid.map((item, i) => (
        <a
          ...
          className={`bg-white rounded-xl border border-neutral-200 hover:shadow-md transition-all overflow-hidden flex flex-col ${accentClass}`}
          dir={isRTL ? 'rtl' : 'ltr'}
        >
          {item.image ? (
            <img
          ...
          <div className="p-4 flex-1 flex flex-col">
            ...
            <p className="font-medium text-neutral-800 leading-snug line-clamp-3 flex-1">
              {item.title}
            </p>
    ...
    <section className="py-12 bg-neutral-50 border-t border-neutral-200">
      <div className="container-app">
        ...
        {loading ? (
          ...
          <ItemGrid
            items={tab === 'syria' ? syriaItems : arabItems}
            isRTL={isRTL}
            accentClass="hover:border-amber-400"
          />
        )}
      </div>
    </section>
